...
st.subheader("Backup / restore calculated projects")
st.caption(
    "Same idea as the client list above: calculated projects also live only in this "
    "browser session and are lost on restart. Export after a batch of calculations, "
    "import at the start of your next session to avoid re-running everything."
)
...
with pc1:
    st.download_button(
        "⬇️ Export projects (JSON)",
        store.export_json(),
        file_name="dmrv_projects.json",
        disabled=len(all_projects) == 0,
    )

with pc2:
    uploaded_projects = st.file_uploader("⬆️ Import projects (JSON)", type="json", key="settings_import_projects")
    if uploaded_projects is not None:
        overwrite_p = st.checkbox("Overwrite existing projects (instead of merging)", key="settings_import_projects_overwrite")
        if st.button("Import projects", key="settings_do_import_projects"):
            try:
                store.import_json(uploaded_projects.read().decode("utf-8"), overwrite=overwrite_p)
