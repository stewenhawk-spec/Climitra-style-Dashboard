...
with st.expander("➕ Add / run a new project", expanded=len(store.get_all_projects()) == 0):
    ...
    if st.button("Run calculation", type="primary"):
        ...
        if not project_id:
            ...
            # --- Commit to the tamper-evident ledger ------------------------
            # One entry per batch (the actual computed row, exactly as it will
            # be shown/exported), plus one entry for the project-level run
            # itself (which config/overrides produced this summary). This is
            # what makes the audit trail immutable: any later edit to a Sheet
            # cell or a re-run with different assumptions is detectable by
            # comparing against what was hashed and signed here.
            for batch_row in result["ledger"].to_dict("records"):
                ledger.append(
                    record_type="PyrolysisBatch",
                    record_id=f"{project_id}:{batch_row['batch_id']}",
                    data=batch_row,
                )

            ledger.append(
                record_type="ProjectRun",
                record_id=project_id,
                data={
                    "summary": result["summary"],
                    "config_overrides": config_overrides or {},
                    "run_timestamp_source": data_source,
                },
            )
            ...
            st.success(
                f"Calculated and saved **{project_id}**. It's now shown on the Home tab, "
                f"and {len(result['ledger'])} batch(es) were committed to the audit ledger."
            )
            st.rerun()
