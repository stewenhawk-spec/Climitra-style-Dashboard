...
if project is None:
    ...
    with info_col:
        ...
        elig_df = ledger["soil_eligible"].value_counts().rename(
            index={True: "Eligible", False: "Ineligible"}
        ).rename_axis("Status").reset_index(name="Batches")
        fig_elig = px.pie(elig_df, names="Status", values="Batches", hole=0.6,
                           color="Status",
                           color_discrete_map={"Eligible": "#1B5E8C", "Ineligible": "#C0392B"})
        fig_elig.update_layout(height=220, margin=dict(t=0, b=0, l=0, r=0),
                                showlegend=True, legend=dict(orientation="h", y=-0.1))
        st.plotly_chart(fig_elig, use_container_width=True)

    st.caption("Full batch ledger and more detailed charts (per-batch emissions breakdown, "
               "QA flags) are on the **Projects** tab.")
